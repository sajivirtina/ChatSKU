...
$is_tag       = is_tag();
$is_search    = is_search();
$is_main_blog = ( ! $is_category && ! $is_tag && ! $is_search );
?>
...
<main id="main" class="chatsku-main blog-archive-main">
    ...
    <section class="blog-hero section-padding" style="padding-bottom: var(--space-12);">
        <div class="container text-center">
            <?php if ( $is_search ) : ?>
                <span class="section-head__eyebrow reveal">Search Results</span>
                <h1 class="section-head__title reveal reveal-delay-1">
                    Results for: <em><?php echo esc_html( get_search_query() ); ?></em>
                </h1>
            <?php elseif ( $is_category ) : ?>
                <span class="section-head__eyebrow reveal">Category</span>
                <h1 class="section-head__title reveal reveal-delay-1"><?php single_cat_title(); ?></h1>
                <?php if ( category_description() ) : ?>
                    <p class="section-head__subtitle reveal reveal-delay-2"><?php echo wp_kses_post( category_description() ); ?></p>
                <?php endif; ?>
            <?php elseif ( $is_tag ) : ?>
                <span class="section-head__eyebrow reveal">Tag</span>
                <h1 class="section-head__title reveal reveal-delay-1"><?php single_tag_title(); ?></h1>
            <?php else : ?>
                <!-- Eyebrow badge — matches Features / FAQ hero style -->
    ...
    <section class="blog-posts-section" style="padding-bottom: var(--space-20);">
        <div class="container">
            <?php if ( have_posts() ) : ?>
                ...
                <?php
                // On main blog listing, pull first post as featured hero card
                $posts_shown = 0;
                if ( $is_main_blog && ! is_paged() ) :
                    the_post();
                    $posts_shown++;
                ...
                <article class="blog-featured-post reveal" <?php post_class(); ?>>
                    <a href="<?php the_permalink(); ?>" class="blog-featured-post__image-link" tabindex="-1" aria-hidden="true">
                        <?php if ( $feat_hero && is_array( $feat_hero ) ) : ?>
                            <img src="<?php echo esc_url( $feat_hero['url'] ); ?>" alt="<?php echo esc_attr( $feat_hero['alt'] ?: get_the_title() ); ?>" class="blog-featured-post__image" loading="eager">
                        ...
                        <?php else : ?>
                            <div class="blog-featured-post__image-placeholder" aria-hidden="true">
                                <svg width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" aria-hidden="true"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                            </div>
                        <?php endif; ?>
                    </a>
                    <div class="blog-featured-post__body">
                        <div class="blog-featured-post__meta">
                            <?php if ( $feat_cat ) : ?>
                                <a class="blog-featured-post__cat" href="<?php echo esc_url( get_category_link( $feat_cat->term_id ) ); ?>"><?php echo esc_html( $feat_cat->name ); ?></a>
                        ...
                        <a href="<?php the_permalink(); ?>" class="chatsku-btn chatsku-btn--secondary chatsku-btn--sm">Read Article →</a>
                    </div>
                </article>

                <?php if ( have_posts() ) : ?>
                    <div class="blog-posts-grid-label reveal">More Articles</div>
                <?php endif; ?>
                <?php endif; /* end featured */ ?>

                <!-- Posts Grid -->
                <?php if ( have_posts() ) : ?>
                    <div class="grid-3 blog-posts-grid">
                        <?php
                        while ( have_posts() ) :
                            the_post();
                            $posts_shown++;
            ...
            <?php else : ?>
                <div class="blog-empty">
                    <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true" style="color: var(--color-text-faint); margin: 0 auto var(--space-5);"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
...
@media (max-width: 900px) {
    ...
    .blog-featured-post__image-placeholder { min-height: 260px; }
}
