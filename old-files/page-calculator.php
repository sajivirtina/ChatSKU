...
function render(){
  ...
  document.getElementById('recSetup').textContent = p.over ? 'from '+money(p.monthly)+'/mo + custom setup' : '+ '+money(setup(p.monthly))+' setup';
...
input[type=range]{-webkit-appearance:none;appearance:none;width:100%;height:5px;border-radius:6px;background:linear-gradient(90deg,var(--amber) var(--p,40%),rgba(159,192,232,.2) var(--p,40%));outline:none;margin-top:18px;}
input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;height:20px;width:20px;border-radius:50%;background:var(--amber);cursor:pointer;border:3px solid var(--navy-900);box-shadow:0 0 0 1px var(--amber);}
input[type=range]::-moz-range-thumb{height:18px;width:18px;border-radius:50%;background:var(--amber);cursor:pointer;border:3px solid var(--navy-900);}
...
.pe-cta{display:block;width:100%;margin-top:18px;background:rgba(244,179,36,.08);border:1px dashed rgba(244,179,36,.55);color:var(--gold);font-family:var(--sans);font-weight:700;font-size:14px;padding:13px 14px;border-radius:11px;cursor:pointer;transition:.15s;}
...
<div class="calc-wrap">
  ...
  <div class="c-panel recover">
    ...
    <button type="button" class="pe-cta" id="peOpen">Not sure of your ChatSKU price? Calculate it &rarr;</button>
    <div class="divider"></div>
    <div class="grid4">
      ...
      <div class="out"><div class="ol">Return on Spend</div><div class="ov green" id="roi">22x</div><div class="osub">per $1 invested</div></div>
    </div>
  </div>
...
(function(){
  ...
  document.getElementById('peApply').addEventListener('click',function(){
    if(latest){
      ...
      moEl.value=num(state.mo);
      setupEl.value=num(state.setup);
      renderAll();
    }
    closeModal();
  });
